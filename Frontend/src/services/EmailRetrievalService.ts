...
export class EmailRetrievalService {
   ...
   * Allows filtering by date ranges for more targeted email management.
   *
   * @param dateFilter Optional date filter to apply
   * @returns Promise resolving to an array of filtered sender groups with email statistics
   * @throws Error if retrieval fails or server returns an error
   */
  async getEmailsBySenderWithFilter(
    ...
    try {
      console.log(
        "Requesting filtered emails grouped by sender from server",
        dateFilter
      );
      const request: EmailsByFilterRequest = {
        DateFilter: dateFilter || null,
      };
      const response = await this.apiClient.post(
        "/email/emails-by-sender/filter",
        request
      );
      const senderGroups: SenderGroup[] = response.data;
      // Sort by email count (highest first) for better user experience
      ...
      console.error(
        "Failed to retrieve filtered emails:",
        error.response?.data?.message || error.message
      );
      throw new Error(
        `Failed to retrieve filtered emails: ${
